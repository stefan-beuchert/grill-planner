...
import { addFoodItem, setFoodSelection } from "@/lib/actions/food";
import { foodItemNameSchema, type FoodItemNameValues } from "@/lib/validations/food";
  ...
  if (!stored) {
    return (
      <div className="flex flex-col gap-2">
        <SectionHeading icon={UtensilsCrossed}>{t.food.heading}</SectionHeading>
        <p className="text-muted-foreground text-sm">{t.food.joinPrompt}</p>
      </div>
    );
  }

  async function changeQuantity(foodItemId: string, quantity: number) {
    if (!stored) return;
    setPendingId(foodItemId);
    await setFoodSelection(slug, stored.participantId, stored.editToken, foodItemId, quantity);
    setPendingId(null);
    router.refresh();
  }
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <Button
                  ...
                  className="h-11 w-11"
                  aria-label={t.food.fewerAria(item.name)}
                >
                ...
                <Button
                  ...
                  onClick={() => changeQuantity(item.id, quantity + 1)}
                  className="h-11 w-11"
                  aria-label={t.food.moreAria(item.name)}
                >
...
function AddFoodItemForm({
  ...
  const schema = useMemo(() => foodItemNameSchema(t), [t]);
  const {
    register,
    handleSubmit,
    reset,
    formState: { errors, isSubmitting },
  } = useForm<FoodItemNameValues>({ resolver: zodResolver(schema) });

  async function onSubmit(values: FoodItemNameValues) {
    setServerError(null);
    const result = await addFoodItem(slug, participantId, editToken, values.name);
    if (!result.success) {
